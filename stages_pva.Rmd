...
Nos podemos preguntar cómo afecta la estocasticidad ambiental a la tasa de crecimiento lambda. El valor determinista de la misma obtenido con `lambda (lotus_matrix)` era 1.02. la librería **popbio** calcula la tasa de crecimiento estocástico mediante dos aproximaciones, usando la función `stoch.growth.rate()`: 

- A partir de *simulación*, utilizando en cada repetición una de las matrices disponibles en **lotus_lista**, 3 en este caso. 
...
Esos valores ligeramente inferiores a 0 corresponden en realidad a la tasa intrínseca de crecimiento *r*, o al logaritmo de lambda estocástica; para obtener lambda tenemos que usar *e* elevado a las tasas simulada y analítica, respectivamente. `exp()` es la función que devuelve el resultado de *e^x^*: 
```{r}
